...
pub enum NodeKind {
  Fokcord {body: Vec<Box<Node>>, id: i64},
...
impl Node {
  pub fn appendToBody(&mut self, node: Node) {
    ...
    match self.kind {
      NodeKind::Fokcord{body: ref mut s ,id:_} => {
        s.push(boxed);
      }
      _ => panic!("AST Error: Tried to append a Node to non-Fokcord Node")
    }
  }
}
