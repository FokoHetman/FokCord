...
impl Interpreter {
  fn evaluate_program(&mut self, prog: AST::Node, env: &mut Environment, program: Fokcord) -> AST::Proventus {
    let mut last_eval = AST::Proventus{..Default::default()};

    match prog.kind {
      AST::NodeKind::Fokcord{body,id} => {
        env.current_node = 0;
        let mut counter = 0;
      ...
      _ => panic!("[Interpreter Error] Tried to evaluate non-Fokcord Node as a Program, {:#?}", prog)
    }
    return last_eval
  }
  pub fn evaluate(&mut self, node: AST::Node, env: &mut Environment, program: Fokcord) -> AST::Proventus {
    match node.kind {
      AST::NodeKind::Fokcord{body:_,id:_} => self.evaluate_program(node, env, program),
  ...
  fn evaluate_matchcase(&mut self, node: AST::Node, env: &mut Environment, program: Fokcord) -> AST::Proventus {
    match node.kind {
  ...
  fn evaluate_if(&mut self, node: AST::Node, env: &mut Environment, program: Fokcord) -> AST::Proventus {
    match node.kind {
  ...
  fn evaluate_bool(&mut self, node: AST::Node, env: &mut Environment, program: Fokcord) -> AST::Proventus {
    match node.kind {
  ...
  fn evaluate_char(&mut self, node: AST::Node, env: &mut Environment, program: Fokcord) -> AST::Proventus {
    match node.kind {
  ...
  fn nest(&mut self, bounds: Vec<(AST::Node, Vec<AST::Proventus>)>, body: Box<AST::Node>, nenv: &mut Environment, program: Fokcord) -> Vec<AST::Proventus> {
    if bounds.len()==0 {
  ...
  fn evaluate_list(&mut self, node: AST::Node, env: &mut Environment, program: Fokcord) -> AST::Proventus {
    match node.kind {
  ...
  fn evaluate_function(&mut self, node: AST::Node, env: &mut Environment, program: Fokcord) -> AST::Proventus {
    match node.kind {
  ...
  fn evaluate_binary_expression(&mut self, node: AST::Node, env: &mut Environment, program: Fokcord) -> AST::Proventus {
    match node.kind {
  ...
  fn evaluate_identifier(&mut self, node: AST::Node, env: &mut Environment, program: Fokcord) -> AST::Proventus {
    ...
    'ma: for variation in env.get(node.clone()) {
        ...
        AST::Fructa::Moenus(args, statement) => {
          ...
          match node.kind {
            AST::NodeKind::Identifier{ref symbol, ref childs} => {
              ...
              env.current_node+=childs.len() as i64;
          ...
          //env.current_node+=args.len() as i64;
      ...
      AST::Fructa::BuiltIn(f, cached_args) => {
        ...
        else if f == builtins::quit || f == builtins::reload_cache {
          fargs = builtins::FunctionArgs::zerumFokcord(program.clone());
        } else if f == builtins::change_server || f == builtins::change_channel {
          if args_vec.len()>0 {
            fargs = builtins::FunctionArgs::singleFokcord(self.evaluate(args_vec[0].clone(), env, program.clone()), program.clone())
          } else {
            fargs = builtins::FunctionArgs::zerumFokcord(program.clone());
          }
        }
  ...
  fn evaluate_object(&mut self, node: AST::Node, env: &mut Environment, program: Fokcord) -> AST::Proventus {
    match node.kind {
