...
fn mul_string(times: i64, string: String) -> String {
...
pub struct Compiler {pub stack_size: i64, pub language: Language}
...
impl Compiler {
  ...
  pub fn parse_c_node(&mut self, node: ANode, current: &mut String, indent: i64, env: &mut Environment) -> String {
  ...
  pub fn inject_label(&mut self, current: String, label: String, indent: i64) -> String {
    format!("{}{label}_start:\n{}", current.split("_start:\n").collect::<Vec<&str>>()[0], current.split("_start:\n").collect::<Vec<&str>>()[1]).to_string()
  }
  pub fn parse_asm_node(&mut self, node: ANode, current: &mut String, indent: i64, env: &mut Environment) -> String {
    let ev_indent = mul_string(indent, String::from(" "));
    //println!("\n\n{:#?}\n```{}```\n\n", node, current);
    match node.kind {
      ANodeKind::NumericLiteral(value) => {
