...
use crate::{KeyEvent, Fokcord, State};
...
pub fn length(arguments: Arguments) -> Proventus {
  match arguments.function {
    FunctionArgs::single(list) => {
      match list.value {
        Fructa::Inventarii(l) => {
          Proventus{value: Fructa::Numerum(l.len() as i64), id: -1}
        }
...
pub fn to_int(arguments: Arguments) -> Proventus {
  match arguments.function {
    FunctionArgs::single(val) => {
      ...
      Proventus {value: Fructa::Numerum(string.parse::<i64>().unwrap()), id: -1}
    }
    _ => panic!("?")
  }
}
...
pub fn quit(arguments: Arguments) -> Proventus {
  match arguments.function {
    FunctionArgs::zerumFokcord(program) => {
...
pub fn change_server(arguments: Arguments) -> Proventus {
  match arguments.function {
    FunctionArgs::zerumFokcord(program) => {
      let mut program = program;
      //program.state = GuildSelection; //!! make it reload known guilds
      Proventus{value: Fructa::FokcordModifier(program), id: -5}
    },
...
pub fn change_channel(arguments: Arguments) -> Proventus {
  match arguments.function {
    FunctionArgs::zerumFokcord(program) => {
      let mut program = program;
      //program.state = State::ChannelSelection; // !! make it reload channels
      Proventus{value: Fructa::FokcordModifier(program), id: -5}
    },
    FunctionArgs::singleFokcord(id, program) => {
      let mut program = program;
...
pub fn reload_cache(arguments: Arguments) -> Proventus {
  match arguments.function {
    FunctionArgs::zerumFokcord(program) => {
      let mut program = program;
      program.reload_cache();
      Proventus{value: Fructa::FokcordModifier(program), id: -5}
    }
    _ => panic!("?")
  }
...
pub fn declare_builtins(env: &mut Environment) {
  let functions = vec![
    ...
    (String::from("server"), change_server),
    (String::from("channel"), change_channel),
    (String::from("reload_cache"), reload_cache),
  ];
  for i in functions {
...
pub enum FunctionArgs {
  ...
  many(Vec<Proventus>),
  fmap(Proventus, Proventus, Environment, Interpreter, Fokcord),
  load_file(Proventus, Environment, Interpreter, Fokcord),

  zerumFokcord(Fokcord),
  singleFokcord(Proventus, Fokcord),
  doubleFokcord(Proventus, Proventus, Fokcord),
}
